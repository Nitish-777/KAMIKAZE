...
export default function ReviewSection({ productId }) {
  ...
  const fetchReviews = async () => {
    ...
    if (res.ok) {
      ...
      // Only show reviews to authenticated users and only if they have purchased
      setReviews(data.canReview ? data.reviews || [] : []);
      setAvgRating(data.canReview ? data.avgRating || 0 : 0);
      setCount(data.canReview ? data.count || 0 : 0);
      ...
      {!isAuthenticated ? (
        <div style={{ textAlign: 'center', padding: '2rem' }}>
          <p style={{ color: 'var(--color-text-muted)', fontSize: '0.9rem' }}>
            Sign in to see reviews and submit your own after receiving this product.
          </p>
        </div>
      ) : canReview ? (
        <>
          ...
          <form onSubmit={handleSubmit} style={{ background: 'var(--color-bg-alt)', border: '1px solid var(--color-border)', borderRadius: 'var(--rounded-md)', padding: '1.25rem', marginBottom: '1.5rem' }}>
            ...
            <div style={{ marginTop: '10px' }}>
              <label style={{ fontSize: '0.85rem', fontWeight: 600, display: 'block', marginBottom: '4px' }}>Attach an Image (optional)</label>
              <input type="file" accept="image/*" onChange={e => setImage(e.target.files[0])} className="input-field" style={{ padding: '6px' }} />
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              {reviews.map(r => (
                <div key={r.id} style={{ padding: '1rem', border: '1px solid var(--color-border)', borderRadius: 'var(--rounded-md)' }}>
                  ...
                  {r.comment && <p style={{ fontSize: '0.9rem', color: 'var(--color-text-muted)', marginBottom: '8px' }}>{r.comment}</p>}
                  {r.imageUrl && (
                    <div style={{ position: 'relative', width: '100px', height: '100px', borderRadius: '4px', overflow: 'hidden', marginTop: '8px', border: '1px solid var(--color-border)' }}>
                      <img src={r.imageUrl} alt="Review" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                    </div>
                  )}
                </div>
              ))}
        ...
        <div style={{ background: 'var(--color-bg-alt)', border: '1px solid var(--color-border)', borderRadius: 'var(--rounded-md)', padding: '1.25rem', marginBottom: '1.5rem', textAlign: 'center' }}>
          <p style={{ color: 'var(--color-text-muted)', fontSize: '0.9rem' }}>
            Only customers who have purchased and received this product can see and write reviews.
          </p>
        </div>
      )}
    </div>
